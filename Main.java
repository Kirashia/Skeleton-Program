...
public class Main {
    ...
    String[] loadAllowedWords()
    {
        String[] allowedWords = {};
        try {
            Path filePath = new File("src\\words\\with\\aqa\\aqawords.txt").toPath();
    ...
    boolean checkWordIsInTiles(String word, String playerTiles)
    {
        ...
        if (!inTiles)
        	Console.print("Word is not in your tile hand");
    ...
    boolean checkWordIsValid(String word, String[] allowedWords)
    {
        ...
        if (!validWord)
        	Console.print("Word is not in the dictionary");
    ...
    void displayTilesInHand(String PlayerTiles, Map tileValues)
    {
    	//string to hold the values of the output
    	String playerHand = "";
    	String playerValues = "";
    	
    	//loops through the hand to find values and indent the string nicely
    	for (char s : PlayerTiles.toCharArray()){
    		playerHand += String.valueOf(s)+ "\t";
    		playerValues += tileValues.get(s) + "\t";
    	}
    	
    	//output
        Console.println();
        Console.println("Hand length: "+PlayerTiles.length()+"\nYour current hand (with values): \n" + playerHand + "\n" + playerValues);
    }
            ...
            int noOfEndOfTurnTiles)
    {
      ...
      displayTilesInHand(playerTiles.playerTiles, tileDictionary);
      String newTileChoice = "2";
      boolean validChoice = false;
      boolean validWord = false;
      
      while (!validChoice || !validWord)
      {
        ...
        else if (!isInteger(choice))
        {
          ...
          {
            Console.println();
            Console.println("This word is not a word");
            Console.println();
            continue;
          }
    ...
    */
    ...
    Main()
    {
      ...
      Console.println();
      Console.println();
      Console.println(allowedWords[0]);
